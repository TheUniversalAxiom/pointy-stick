...
type Intent = 'summary' | 'plan' | 'risks' | 'decision' | 'general'

function inferIntent(userText: string): Intent {
  const text = userText.toLowerCase()
  if (text.includes('summar') || text.includes('overview')) return 'summary'
  if (text.includes('plan') || text.includes('roadmap') || text.includes('steps')) return 'plan'
  if (text.includes('risk') || text.includes('concern') || text.includes('mitigation')) return 'risks'
  if (text.includes('choose') || text.includes('decide') || text.includes('option')) return 'decision'
  return 'general'
}

function buildSonnyReply(
  userText: string,
  model: string
): { reply: string; intelligence: number } {
  ...
  const narrativeIntro =
    modelTone === 'mentor'
      ? 'Here is a grounded, supportive read on your directive.'
      : modelTone === 'analyst'
      ? 'Analyzing the signal and converting it into a structured response.'
      : 'Strategic brief assembled from the current signal.'

  const guidanceMap: Record<Intent, string[]> = {
    summary: [
      `Summary: ${trimmed || 'Awaiting your directive.'}`,
      'Signal strength is stable; focus on clarity and scope.',
      'If you want more precision, share constraints, target audience, and timeline.'
    ],
    plan: [
      'Plan: define objectives → map constraints → draft milestones → assign owners → review risk.',
      'Start with a 48-hour sprint to validate assumptions.',
      'Share success criteria to tune the next phase.'
    ],
    risks: [
      'Risks: unclear scope, missing dependencies, and time compression.',
      'Mitigations: tighten requirements, confirm owners, and align on checkpoints.',
      'Identify the highest-impact unknown and resolve it first.'
    ],
    decision: [
      'Decision frame: list options, score impact vs. effort, and pick the dominant path.',
      'If options are close, favor the one that shortens feedback cycles.',
      'Provide constraints to narrow the recommendation.'
    ],
    general: [
      `Input focus: “${trimmed || 'Awaiting your directive'}”`,
      'I can convert this into a plan, summary, risks, or next actions.',
      'Tell me the outcome you want, and I will lock the next move.'
    ]
  }

  const guidanceLines = guidanceMap[intent]
  const metricsBlock = [
    '',
    'Axiom snapshot',
  ...
  const reply = [narrativeIntro, '', ...guidanceLines, ...metricsBlock].join('\n')
...
export function SonnyChatbotModule({ onBackToMenu }: SonnyChatbotModuleProps) {
  ...
  const handleSend = () => {
    ...
    const { reply: sonnyCoreReply } = buildSonnyReply(input, selectedModel)
